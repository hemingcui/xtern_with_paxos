...
void paxq_create_shared_mem() {
  ...
  *nodeRole = ROLE_INVALID;
  ...
  lockFileFd = open(lockFilePath.c_str(), O_CREAT | O_RDWR, 0x666);
  if (lockFileFd == -1) {
    std::cout << "paxq_create_shared_memory file lock " << lockFilePath << " created failed, errno " << errno << ".\n";
    exit(1);
  }
}
...
void paxq_open_shared_mem(int node_id) {
  ...
  lockFileFd = open(lockFilePath.c_str(), O_RDWR, 0x666);
  if (lockFileFd == -1) {
    std::cout << "paxq_open_shared_memory file lock " << lockFilePath << " open2 failed, errno " << errno << ".\n";
    exit(1);
  }
}
